...
<input type="image" src="${url_context}<spring:theme code="db_remove"/>" title="<spring:message code="label.delete"/>"
		onclick="IDEYATECH.crud.confirmDelete('${prefix}-row-${id}','${url_context}/${page}?action=delete&codeId=${id}${urlParam}',
				'${title}');"/>